...
|  <code>handler</code> | [`ObsidianProtocolHandler`](ObsidianProtocolHandler) | <p>the callback to trigger. A key-value pair that is decoded from the query will be passed in. For example, <code>obsidian://open?key=value</code> would generate <code>{'action': 'open', 'key': 'value'}</code>.</p><p> 0.11.0</p> |
